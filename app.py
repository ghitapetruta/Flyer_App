...
from flyer_engine import (
    ...
    img_bytes_to_b64, generate_html, build_pptx, html_to_pdf,
)
    ...
    if generate_btn:
        ...
        if not produse:
            ...
            with st.spinner("Generez flyerul..."):
                ...
                # Generăm PDF folosind WeasyPrint
    ...
    if "last_html" in st.session_state and st.session_state.last_html:
        ...
        with col_dl1:
            if st.session_state.get("last_pdf"):
                st.download_button(
                    ...
                    file_name=f"{st.session_state.last_filename}.pdf",
                    mime="application/pdf",
                    use_container_width=True,
                    help="PDF gata de printat sau trimis prin email.",
                )
            else:
                st.warning("⚠️ PDF indisponibil")
                st.caption("Lipsește WeasyPrint sau dependențele sale sistem.")
        ...
        with col_dl2:
            st.download_button(
                ...
                mime="text/html",
                use_container_width=True,
                help="HTML editabil. Deschide în browser pentru previzualizare.",
            )
